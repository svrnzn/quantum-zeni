...
def run_simulation(params):
    ...
    for i in range(params.Nt):
        traj = dimer.run(params.state0, params.T)
        trajectories.append(traj[int(t_steps/2)::del_t_steps])
        print(f"Trajectory #{i+1} of {params.Nt} computed.")

    f = open(f"data/dimer-myte-lmbd_1={params.lmbd_1:.1f}-lmbd_2={params.lmbd_2:.1f}-Nt={params.Nt}-T={params.T}-dt={params.dt}.pkl", "wb")
